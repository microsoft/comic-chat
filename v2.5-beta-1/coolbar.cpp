	...
	DWORD dwAfxBarStyle, UINT nID)
{
	...
	return CWnd::CreateEx(WS_EX_TOOLWINDOW, REBARCLASSNAME, NULL,
		dwStyle, 0, 0, 0, 0, pParentWnd->GetSafeHwnd (), (HMENU)nID);
}
...
void CFlatOrCoolBarCmdUI::Enable(BOOL bOn)
{
	...
	BOOL bWasDisabled = (nNewStyle & TBBS_DISABLED);
	nNewStyle = nNewStyle & ~TBBS_DISABLED;
	if (!bOn)
	{
...
CRebarBandInfo::CRebarBandInfo() 
{
	ZeroMemory (this, sizeof(*this));
   #if defined(_WIN32_IE) && (_WIN32_IE >= 0x0400)
	if (IsComCtlNewerThan (COMCTLVER(4,71)))
		cbSize = sizeof(REBARBANDINFO);
	else
		cbSize = REBARBANDINFO_V3_SIZE;
   #else
	cbSize = sizeof(REBARBANDINFO);
   #endif
}
