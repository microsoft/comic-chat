...
STDMETHODIMP CDocObjectServerDoc::XDocOleObject::Close(DWORD dwSaveOption)
{
	...
	// clear user control so server will shutdown if this is the last doc
	if( AfxGetApp()->m_pDocManager->GetOpenDocumentCount() <= 1 )
	{
		AfxOleSetUserCtrl( FALSE );
		g_bOleShuttingDown = TRUE; // tell CChatApp::PreTranslateMessage to catch
								   // weird broken MDI windows problem
	...
	HWND hwndParent, LPCRECT lpPosRect)
{
	...
	try
		...
		if( iVerb == OLEIVERB_UIACTIVATE )
			hr = pThis->RestoreMenu();
		else
			hr = pThis->m_xOleObject.
				DoVerb(iVerb, lpmsg, pActiveSite, lindex, hwndParent, lpPosRect);
		}
	catch( ... )
		{/* we don't care, man */}
