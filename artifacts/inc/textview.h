...
const short g_nHighlightedFormats			= 8;
...
public:
	...
	BOOL	bSetDefaultHighlightFormat(CHARFORMAT *pCharFormat, SHORT nIndex);

	BOOL	bSetMessageFormat(CHARFORMAT *pCharFormat, MSG_TYPE MsgType, 
								MEMBER_STATUS MembFrom = msParticipant, 
								BOOL bHeader = FALSE);

	BOOL	bSetHighlightFormat(CHARFORMAT *pCharFormat, SHORT nIndex);

	BOOL	bGetDefaultMessageFormat(CHARFORMAT **ppCharFormat, MSG_TYPE MsgType, BOOL bHeader);

	BOOL	bGetDefaultHighlightFormat(CHARFORMAT **ppCharFormat, SHORT nIndex);

	BOOL	bGetMessageFormat(CHARFORMAT **ppCharFormat, MSG_TYPE MsgType, 
								MEMBER_STATUS MembFrom = msParticipant, 
								BOOL bHeader = FALSE);

	BOOL	bGetHighlightFormat(CHARFORMAT **ppCharFormat, SHORT nIndex);
	...
	BOOL    bGetLastAutoScroll() { return m_bLastAutoScroll; }
...
//	BOOL	bSetFormat(PARAFORMAT *pParaFormat);
	...
	BOOL	bReSetDefaultHighlightFormats(BOOL bReset = TRUE);
...
//	BOOL	bSaveAndMoveSel(CHARRANGE *pcr);
	...
	// Sets the default formats for highlighted texts
	BOOL	bSetDefaultHighlightFormats();

	// Delete any allocated Msg Type Properties
	void	ClearDefaultMsgTypeProperties(void);

	// Delete any allocated CHARFORMATs for highlighted formats
	void	ClearDefaultHighlightFormats(void);
...
private:
	...
	CHARFORMAT*		m_pcfHighlight;		// font used for highlighted messages

	short			m_nInsertBlank;		// Flag for inserting blank lines
	short			m_nAutoScroll;		// Flag for AutoScroll

	BOOL			m_bNewBrowser;		// Determine where to launch URLs
...
//	USHORT			m_rguResStatus[msEndEnum];		// Resource IDs for the status headers "Host ..."
	...
	CHARFORMAT		*m_rgpcfHighlights[g_nHighlightedFormats];
	CHARFORMAT		m_rgcfDefHighlights[g_nHighlightedFormats];
};
