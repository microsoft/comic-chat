...
#define FRAMERWND_SETUIACTIVE  (WM_USER+1)
...
protected:
	...
	HWND			m_hWndMenu;			//UI Active object's window to relay menus commands to
	...
	CComPtr<ICChatAutomation> m_pCChatCxx; // CChat's dual interface so we can get at ShowMenu
	...
	BOOL	 m_bDocObjActivated;
	BOOL	 m_bClientSiteClosing;
	BOOL	 m_bClientSiteDestroyed;
...
public:
	...
	void SetIsClosing( void )
		{ m_bIsClosing = TRUE; }
	void NotifyDestroyClientSite()
		{ m_bClientSiteDestroyed = TRUE; }
	BOOL IsClientSiteDestroyed()
		{ return m_bClientSiteDestroyed; }
	void WaitDestroyClientSite();

	void AttachToWindow(HWND hwnd)
		{ if (hwnd) ::SetWindowLong (hwnd, FRAMEWL_STRUCTURE, (LONG)this); }
	void DetachFromWindow(HWND hwnd)
		{ if (hwnd) ::SetWindowLong (hwnd, FRAMEWL_STRUCTURE, 0); }
	static CFrame* GetFromWindow(HWND hwnd)
		{ 
			CFrame * pFrame = (CFrame *)::GetWindowLong (hwnd, FRAMEWL_STRUCTURE); 
			return pFrame;
		}
