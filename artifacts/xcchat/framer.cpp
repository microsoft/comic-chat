...
CFrame::CFrame(HINSTANCE hInst, HINSTANCE hInstPrev, LPSTR pszCmdLine )    
{
	...
	m_bDocObjActivated = FALSE;
	m_bClientSiteClosing = FALSE;
	m_bClientSiteDestroyed = FALSE;
}
...
void 
CFrame::PreDelete(void)
{
	ATLTRACE( "CFrame::PreDelete\n" );

	if( m_pSite != NULL && !m_bClientSiteClosing)
		Close();				
	
	// Shutdown wndprocs.
	DetachFromWindow (m_hWnd);
	DetachFromWindow (m_hWndClient);
...
LRESULT CFrame::OnCommand(HWND hWnd, WPARAM wParam, LPARAM lParam)
{
	...
	if(this != NULL && m_hWndMenu != NULL)
	{
		// relay command to doc object
		ATLTRACE( "relaying command %x:%x to doc object\n", wParam, lParam );
		// Let's send the message with a more deadlock-avoiding function,
		// SendNotifyMessage, instead of SendMessage
		return( SendNotifyMessage( m_hWndMenu, WM_COMMAND, wParam, lParam ) );
	}
	else
	{
		// no doc object to talk to, eat command
		ATLTRACE( "eating command %x:%x\n", wParam, lParam );
		return 0L;
	}
}
...
{    
	...
	// if this is CChat we're talking to (better be...) get its C++ interface
	m_pSite->GetObject ()->QueryInterface( IID_ICChatAutomation, (void **)&m_pCChatCxx );
	if( m_pCChatCxx == NULL )
	{	
		ATLTRACE( " ***Can't get C++ interface to control menus, must not be CChat! ***\n" );
		return( FALSE );
	}

	m_bDocObjActivated = FALSE; // will be activated during first ResetSharedMenu call
...
void CFrame::ResetSharedMenu( void )
{
	...
	if( !m_bDocObjActivated )
   	{
		ATLTRACE( "activating doc object\n" );
		m_bDocObjActivated = TRUE;
...
void CFrame::RemoveSharedMenu( void )
{
	...
	if( !m_bDocObjActivated )
	{
		ATLTRACE( "doc object not activated yet, ignoring call\n" );
		return;
	}
...
IViewObject * CFrame::GetViewObject( void )
{
							   ...
							   LPARAM lParam )
{
	...
	if (iMsg == WM_CREATE)
	{
		pFrame = (CFrame *)(((LPCREATESTRUCT)lParam)->lpCreateParams);
		_ASSERTE (pFrame != NULL);
		pFrame->AttachToWindow (hWnd);
	}
	else
	{
		pFrame = CFrame::GetFromWindow (hWnd);
	}
    ...
    switch (iMsg)
    {
        ...
        case WM_SIZE:
			...
			if( pFrame->m_bIsClosing )
				break;

            //Tell the in-place object about the new frame size
            GetClientRect(hWnd, &rc);

            if (NULL != pFrame->m_pIOleIPActiveObject)
                pFrame->m_pIOleIPActiveObject->ResizeBorder(&rc, pFrame, TRUE);
			 ...
			 */
            rc.left   += pFrame->m_bwIP.left;
            rc.right  -= pFrame->m_bwIP.right;
            rc.top    += pFrame->m_bwIP.top;
            rc.bottom -= pFrame->m_bwIP.bottom;            

			pFrame->ResizeClientWindow( rc.left, rc.top, rc.right-rc.left, rc.bottom-rc.top );
        ...
        case WM_SETFOCUS:
            ...
            if (NULL != pFrame->m_pIOleIPActiveObject)
            {
                ...
                pFrame->m_pIOleIPActiveObject->GetWindow(&hWndObj);
				SetFocus(hWndObj);
        ...
        case FRAMERWND_SETUIACTIVE:
			ATLTRACE( "FrameWndProc - FRAMERWND_SETUIACTIVE\n" );

			//set shared menu back up
			pFrame->ResetSharedMenu();
			break;


		case WM_TIMER:		
			ATLTRACE( "FrameWndProc - WM_TIMER\n" );
			if( pFrame->m_bDelayedOpenNM && (!pFrame->m_bIsClosing) )
			{
				HWND hwndNM;
				...
				if( (hwndNM = ::FindWindow( CONF_CLASS, NULL )) != NULL )
				{
					...
					pFrame->m_bDelayedOpenNM = FALSE; // only do this once
				}
				else if( (hwndNM = ::FindWindow( CONF_TRAYCLASS, NULL )) != NULL)
				{
		...
		case WM_COMMAND:
			pFrame->OnCommand( hWnd, wParam, lParam);
			break;
...
{    
	...
	Fi.cb = sizeof( OLEINPLACEFRAMEINFO );		// don't forget to set size

	CComPtr<IOleInPlaceFrame>    pIIPFrame;		// CComPtr takes care of Release()
	CComPtr<IOleInPlaceUIWindow> pIIPUIWindow;
...
STDMETHODIMP CFrame::SetMenu(HMENU hMenu, HOLEMENU hOLEMenu, HWND hWndObj)
{
	...
	ATLTRACE( "setting m_hWndMenu to doc object hWnd = %x\n", hWndObj );
	m_hWndMenu=hWndObj; // save objects hwnd for relaying menu commands

	m_spInPlaceSite->GetWindowContext( &pIIPFrame, &pIIPUIWindow, &rcPos, &rcClip, &Fi );

	// Give control container XCChat's frame window so menu commands can be relayed to object.
	// This completely isolates object from control container and solves numerous shutdown problems
	return( pIIPFrame->SetMenu( hMenu, hOLEMenu, m_hWnd ) );
}
...
STDMETHODIMP CFrame::RemoveMenus(HMENU hMenu)
{
	...
	ATLTRACE( "setting m_hWndMenu to NULL\n" );
	m_hWndMenu=NULL; // stop relaying menu commands

	try
	{
		m_spInPlaceSite->GetWindowContext( &pIIPFrame, &pIIPUIWindow, &rcPos, &rcClip, &Fi );
		hr = pIIPFrame->RemoveMenus( hMenu );
	}
	catch(...)
	{
		ATLTRACE( "***IGNORING FAULT FROM m_spInPlaceSite->GetWindowContext***" );
		hr = S_OK;
	}
...
STDMETHODIMP CFrame::SetStatusText(LPCOLESTR pszText)
{
	...
	if( m_bIsClosing )
		return( S_OK ); // don't do this when closing (...BOOM...)
...
void
CFrame::WaitDestroyClientSite()
{
	while (!m_bClientSiteDestroyed && m_pSite != NULL) 
	{
		m_pSite->TestServerShutdownAndClose ();
	}
}
