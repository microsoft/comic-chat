...
class MultiarrayOperand extends InsnOperand implements VMConstants
{
  ClassCP cp;
  int sz;

  MultiarrayOperand(DataInput in, CP cps[])
       throws IOException
  {
    cp = (ClassCP) cps[in.readUnsignedShort()];
    sz = in.readUnsignedByte();
  }
  void makeCP(Environment env, ClassInfo cinfo)
  {
    cp.setNewName(env, cinfo);
    cp = (ClassCP) cinfo.addCP(cp);
  }
