	...
	if (bIsIrcXSocket())
	{
		...
		switch (uCode)
		{
			...
			case ERR_SECURITY:
				...
				// involved 
			case ERR_NOACCESS:
				// ":<ServerName> 913 <Nickname> <*|ChannelName> :No access"
			...
			case ERR_NOTSUPPORTED:
				// ":<ServerName> 925 <Nickname> <ObjectName> :Command not supported by object"
				hr = CC_E_UNKNOWNCOMMAND;
				break;

			case ERR_CHANNELEXIST:
				// ":<ServerName> 926 <Nickname> <ChannelName> :Channel already exists."
				hr = CC_E_CHANNELALREADYEXISTS;
				break;
	...
	if (ERR_NOTONCHANNEL == uCode || ERR_NOSUCHCHANNEL == uCode)
	{
										  ...
										  NULL))
		{
			...
			case RPL_CHANNELMODEIS:
				m_lModeIsInProgress--;
				ASSERT(m_lModeIsInProgress >= 0L, "m_lModeIsInProgress < 0L in CMsChatPr::HandleErrors");
				break;
				default:
	...
	if (bIsIrcXSocket())
		switch (uCode)
		{
			...
			case ERR_NOACCESS:
				...
				if (_tcscmp("*", pPrIrcMsg->szParams[1]))
					szChannelName = pPrIrcMsg->szParams[1];
				break;
		}
	else
...
void CMsChatPr::ExposeChannelItems(CChatChannel* pChannel, PPRCHANNEL pPrChannel, BOOL bFreeContent)
{
	BOOL		bRet;
	CChatItems	*pCItems  = NULL;
...
void CMsChatPr::FireConnectionState(enumConnectionState csNewConnectionState)
{
	...
	if (csDisconnected == csNewConnectionState)
	{
		...
		ASSERT(bRet, "ListType's bDeleteList() failed in CMsChatPr::FireConnectionState");

		bRet = m_listMask.bDeleteList();
		ASSERT(bRet, "Mask's bDeleteList() failed in CMsChatPr::FireConnectionState");

		bRet = m_listRedir.bDeleteList();
		ASSERT(bRet, "Redir's bDeleteList() failed in CMsChatPr::FireConnectionState");
...
BOOL CMsChatPr::bFireChannelProperty(CChatChannel* pChannel, PPRCHANNEL pPrChannel)
{
	// Just a redirector to ExposeChannelItems
	OutputDebugThreadIdString("CMsChatPr::bFireChannelPropertyChanged - Enter\n");
	ASSERT(pChannel, "pChannel is NULL in CMsChatPr::bFireChannelProperty");
	ASSERT(pPrChannel, "pPrChannel is NULL in CMsChatPr::bFireChannelProperty");

	ExposeChannelItems(pChannel, pPrChannel, FALSE /*bFreeContent*/);
	return TRUE;
}
