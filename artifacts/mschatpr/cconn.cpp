...
HRESULT CChatConn::HrAuthenticate(CHAR *szUserName, CHAR *szPassword, CHAR *szSecurityPackage)
{
    ...
    while (SUCCEEDED(HrIsSocketValid()))
    {
        ...
        if (cmdidAuth != nCmd)
		{
			...
			if (0 == lstrcmp(prIrcMsg.szCommand, "910"))
			{
				m_bAuthFailed = TRUE;
				hr = CC_E_AUTHLOOP;
			}
...
HRESULT CChatConn::HrGenerateAndSendAuthMsg(CHAR *szBlob, CHAR *szSecurityPackage)
{
    ...
    if (szBlob == NULL)
    {
		...
		ASSERT(!m_bContext, "m_bContext is TRUE in CChatConn::HrGenerateAndSendAuthMsg");
        phCurrContext = NULL;
        pInSecDesc = NULL;
    }
    else
	...
	// First try using an existing session key
	if (m_bAuthFailed)
	{
