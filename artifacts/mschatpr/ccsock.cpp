...
CChatSocket::~CChatSocket(void)
{
	...
	bRet = m_listListType.bDeleteList();
	ASSERT(bRet, "ListType's bDeleteList() failed in CChatSocket::~CChatSocket");

	bRet = m_listMask.bDeleteList();
	ASSERT(bRet, "Mask's bDeleteList() failed in CChatSocket::~CChatSocket");

	bRet = m_listRedir.bDeleteList();
	ASSERT(bRet, "Redir's bDeleteList() failed in CChatSocket::~CChatSocket");

	bRet = m_listChannel.bDeleteList();
...
HRESULT CChatSocket::HrLogin(void)
{
	...
	return HrIterateLogin(pPrSecurity, CC_E_LOGINFAILED, TRUE /*bForceNextPackage*/);
}
...
//=--------------------------------------------------------------------------=
HRESULT CChatSocket::HrIterateLogin(PPRSECURITY pPrSecurity, HRESULT hrLatest, BOOL bForceNextPackage)
{
	HRESULT			hr = NOERROR;
	HRESULT			hrLogin = NOERROR;
	LPTSTR			szPackageIndex = NULL;
	LPTSTR			szPackageAvailable = NULL;
	static TCHAR	szPackageToTry[g_nMaxLengthSmall+1] = { 0 };
	...
	{
		// We're looping because user gave wrong info -> display error message box
		TCHAR	szMessage[2*g_nMaxLengthSmall+1];

		if (::LoadString(_Module.GetModuleInstance(), IDS_PR_WRONGAUTHDATA, szMessage, 2*g_nMaxLengthSmall))
			::MessageBox(NULL, (LPCTSTR) szMessage, NULL /*(LPCTSTR) szTitle*/, MB_ICONEXCLAMATION | MB_OK | MB_TASKMODAL | MB_TOPMOST);
	}
	...
	if (!lstrcmpi(szPackageToTry, g_szAnon))
	{
		if (pPrSecurity->bAnonAllowed)
		{
			...
			if (SUCCEEDED(hr = HrTryLogin(NULL)))
				...
				return HrIterateLogin(pPrSecurity, hr, TRUE /*bForceNextPackage*/);
		}
		else
			return HrIterateLogin(pPrSecurity, hrLatest, TRUE /*bForceNextPackage*/);
	}
	...
	if (pPrSecurity->cPackages > 0)
	{
		...
		if (cCnt <= pPrSecurity->cPackages)
		{
			...
			bLoopPwd = !lstrcmpi(szPackageAvailable, g_szMSN) || !lstrcmpi(szPackageAvailable, g_szDPA);
			if (SUCCEEDED(hr = HrTryLogin(szPackageAvailable)))
				return NOERROR;
			else
				return HrIterateLogin(pPrSecurity, hr, hr != CC_E_AUTHLOOP /*bForceNextPackage*/);
		}
		else
			// this package is not available on the server, try the next one
			return HrIterateLogin(pPrSecurity, CC_E_AUTHNOTAVAIL, TRUE /*bForceNextPackage*/);
	}
	else
	...
	switch (cpnProp)
	{
		...
		case cpnMemberCount:
			...
			if (SUCCEEDED(m_hrLastError))
			{
				...
				m_lListInProgress++;
				m_listListType.bInsertCell((LONG) m_listListType.DwGetCellCount()+1, (PVOID) g_byteChannelPropAccessType);
				ASSERT(m_listListType.DwGetCellCount() == (DWORD) (m_lListInProgress + m_lChannelListingInProgress), "m_listListType.DwGetCellCount() == m_lListInProgress + m_lChannelListingInProgress in CChatSocket::bQueryChannelProperty");
			}
			break;
...
error:
	...
	if (pPrQuery)
		...
		{
			// We now wait for the answer asynchronously
...
BOOL CChatSocket::bGetChannelNameFromVariant(VARIANT *pvChannelName, LPTSTR *pszChannelName)
{
	...
	if ((pvChannelName->vt != VT_BSTR || (pvChannelName->vt == VT_BSTR && !pvChannelName->bstrVal)) 
		&& (m_lOpenChannels != 1L))
	{
...
BOOL CChatSocket::bListChannels(CChatItems *pCItems)
{
	...
	if (SUCCEEDED(m_hrLastError))
	{
		...
		if (SUCCEEDED(m_hrLastError))
		{
			...
			m_listListType.bInsertCell((LONG) m_listListType.DwGetCellCount()+1, (PVOID) g_byteChannelListingType);
			ASSERT(m_listListType.DwGetCellCount() == (DWORD) (m_lListInProgress + m_lChannelListingInProgress), "m_listListType.DwGetCellCount() == m_lListInProgress + m_lChannelListingInProgress in CChatSocket::bListChannels");
		}
	}
...
BOOL CChatSocket::bFillChatItemsFromChannel(CChatItems *pCItems, PPRCHANNEL pPrChannel)
{
	...
	LPWSTR wszChannelProp;

	// Set the OnJoin property
	if (pPrChannel->szOnJoin && g_chEOS != pPrChannel->szOnJoin[0])
	{
		ASSERT(!pPrChannel->bMicOnly, "pPrChannel->bMicOnly is TRUE in CChatSocket::bFillChatItemsFromChannel");

		if (g_chExtChnPfx == pPrChannel->szName[0])
		{
			// Need to convert UTF8 extended channel onjoin to Unicode
			if (!bConvertUTF8StringToWide(pPrChannel->szOnJoin, 0 /*cchIn*/, &wszChannelProp, NULL /*pcchOut*/, FALSE /*bNickname*/, FALSE /*bChannelName*/, FALSE /*bPostProcess*/))
			{
				m_hrLastError = E_OUTOFMEMORY;
				return FALSE;
			}
			ASSERT(wszChannelProp, "wszChannelProp in NULL in CChatSocket::bFillChatItemsFromChannel");

			szTmp = W2T(wszChannelProp);
			delete [] wszChannelProp;
		}
		else
		{
			// Might have to convert onjoin from Jis to ShiftJis, etc...
			if (!bConvertString(TRUE, m_byteCharSet, pPrChannel->szOnJoin, 0, &szTmp, NULL, &bFreeConverted))
			{
				m_hrLastError = E_OUTOFMEMORY;
				return FALSE;
			}
		...
		if (!pCItems->bSetItem(g_rgwszChannelPropertyNames[cpnOnJoin], szTmp, 0L))
		{
			m_hrLastError = pCItems->HrGetLastError();
			return FALSE;
		}
	...
	if (pPrChannel->szOnPart && g_chEOS != pPrChannel->szOnPart[0])
	{
		...
		// Might have to convert onjoin from Jis to ShiftJis, etc...
		if (!bConvertString(TRUE, m_byteCharSet, pPrChannel->szOnPart, 0, &szTmp, NULL, &bFreeConverted))
		{
			m_hrLastError = E_OUTOFMEMORY;
			return FALSE;
		}

		if (!pCItems->bSetItem(g_rgwszChannelPropertyNames[cpnOnPart], szTmp, 0L))
		{
			m_hrLastError = pCItems->HrGetLastError();
			return FALSE;
		}
	...
	if (pPrChannel->szSubject && g_chEOS != pPrChannel->szSubject[0])
	{
		...
		if (g_chExtChnPfx == pPrChannel->szName[0])
		{
			// Need to convert UTF8 extended channel subject to Unicode
			if (!bConvertUTF8StringToWide(pPrChannel->szSubject, 0 /*cchIn*/, &wszChannelProp, NULL /*pcchOut*/, FALSE /*bNickname*/, FALSE /*bChannelName*/, FALSE /*bPostProcess*/))
			{
				m_hrLastError = E_OUTOFMEMORY;
				return FALSE;
			}
		...
		{
			// Might have to convert subject from Jis to ShiftJis, etc...
			if (!bConvertString(TRUE, m_byteCharSet, pPrChannel->szSubject, 0, &szTmp, NULL, &bFreeConverted))
			{
				m_hrLastError = E_OUTOFMEMORY;
				return FALSE;
			}
		...
		if (!pCItems->bSetItem(g_rgwszChannelPropertyNames[cpnSubject], szTmp, 0L))
		{
			m_hrLastError = pCItems->HrGetLastError();
			return FALSE;
		}
	...
	if (pPrChannel->szOwnerKey && g_chEOS != pPrChannel->szOwnerKey[0])
	{
		...
		if (g_chExtChnPfx == pPrChannel->szName[0])
		{
			// Need to convert UTF8 extended channel ownerkey to Unicode
			if (!bConvertUTF8StringToWide(pPrChannel->szOwnerKey, 0 /*cchIn*/, &wszChannelProp, NULL /*pcchOut*/, FALSE /*bNickname*/, FALSE /*bChannelName*/, FALSE /*bPostProcess*/))
			{
				m_hrLastError = E_OUTOFMEMORY;
				return FALSE;
			}
		...
		{
			// Might have to convert ownerkey from Jis to ShiftJis, etc...
			if (!bConvertString(TRUE, m_byteCharSet, pPrChannel->szOwnerKey, 0, &szTmp, NULL, &bFreeConverted))
			{
				m_hrLastError = E_OUTOFMEMORY;
				return FALSE;
			}
		...
		if (!pCItems->bSetItem(g_rgwszChannelPropertyNames[cpnOwnerKey], szTmp, 0L))
		{
			m_hrLastError = pCItems->HrGetLastError();
			return FALSE;
		}
	...
	if (pPrChannel->szHostKey && g_chEOS != pPrChannel->szHostKey[0])
	{
		...
		if (g_chExtChnPfx == pPrChannel->szName[0])
		{
			// Need to convert UTF8 extended channel hostkey to Unicode
			if (!bConvertUTF8StringToWide(pPrChannel->szHostKey, 0 /*cchIn*/, &wszChannelProp, NULL /*pcchOut*/, FALSE /*bNickname*/, FALSE /*bChannelName*/, FALSE /*bPostProcess*/))
			{
				m_hrLastError = E_OUTOFMEMORY;
				return FALSE;
			}
		...
		{
			// Might have to convert hostkey from Jis to ShiftJis, etc...
			if (!bConvertString(TRUE, m_byteCharSet, pPrChannel->szHostKey, 0, &szTmp, NULL, &bFreeConverted))
			{
				m_hrLastError = E_OUTOFMEMORY;
				return FALSE;
			}
		...
		if (!pCItems->bSetItem(g_rgwszChannelPropertyNames[cpnHostKey], szTmp, 0L))
		{
			m_hrLastError = pCItems->HrGetLastError();
			return FALSE;
		}
	...
	{
		if (g_chExtChnPfx == pPrChannel->szName[0] && !pPrChannel->bMicOnly)
		{
			// Need to convert UTF8 extended channel keyword to Unicode
			if (!bConvertUTF8StringToWide(pPrChannel->szKeyword, 0 /*cchIn*/, &wszChannelProp, NULL /*pcchOut*/, FALSE /*bNickname*/, FALSE /*bChannelName*/, FALSE /*bPostProcess*/))
			{
				m_hrLastError = E_OUTOFMEMORY;
				return FALSE;
			}
		...
		{
			// Might have to convert keyword from Jis to ShiftJis, etc...
			if (!bConvertString(TRUE, m_byteCharSet, pPrChannel->szKeyword, 0, &szTmp, NULL, &bFreeConverted))
			{
				m_hrLastError = E_OUTOFMEMORY;
				return FALSE;
			}
		...
		if (!pCItems->bSetItem(g_rgwszChannelPropertyNames[cpnKeyword], szTmp, 0L))
		{
			m_hrLastError = pCItems->HrGetLastError();
			return FALSE;
		}
	...
	if (pPrChannel->szTopic)
	{
		if (pPrChannel->szTopic[0] && g_chExtChnPfx == pPrChannel->szName[0] && !pPrChannel->bMicOnly)
		{
			...
			if (!bConvertUTF8StringToWide(pPrChannel->szTopic, 0 /*cchIn*/, &wszChannelProp, NULL /*pcchOut*/, FALSE /*bNickname*/, FALSE /*bChannelName*/, FALSE /*bPostProcess*/))
			{
				m_hrLastError = E_OUTOFMEMORY;
				return FALSE;
			}
	...
	ASSERT(pPrChannel->szName, "pPrChannel->szName is NULL in CChatSocket::bFillChatItemsFromChannel");
	...
	if (bIsIrcXSocket())
	{
		if (g_chExtChnPfx == pPrChannel->szName[0])
		{
			...
			if (!bConvertUTF8StringToWide(pPrChannel->szName, 0 /*cchIn*/, &wszChannelProp, NULL /*pcchOut*/, FALSE, TRUE))
			{
				m_hrLastError = E_OUTOFMEMORY;
				return FALSE;
			}
...
BOOL CChatSocket::bHandleTextMsg(PPRIRCMSG pPrIrcMsg, SHORT nCmd)
{
	...
	BOOL				bAuthUser;
	LPWSTR				wszChannelName;
	BOOL				bRet, bFreeConverted = FALSE;
		...
		0 == ::lstrcmpi(pPrIrcMsg->szParams[0], "*"))	// This is a Sysop broadcast
	{
		...
		else
		{
			...
			if (bIsIrcXSocket() && 
				(g_chExtChnPfx == szSender[0] || g_chGblChnPfx == szSender[0] || g_chLclChnPfx == szSender[0]))
			{
				if (g_chExtChnPfx == szSender[0])
				{
					// Need to convert UTF8 extended channel name to Unicode
					if (!bConvertUTF8StringToWide(szSender, 0 /*cchIn*/, &wszChannelName, NULL /*pcchOut*/, FALSE, TRUE))
					{
						m_hrLastError = E_OUTOFMEMORY;
						return FALSE;
					}
					ASSERT(wszChannelName, "wszChannelName in NULL in CChatSocket::bHandleTextMsg");
					szSender = W2T(wszChannelName);
					delete [] wszChannelName;
				}
				else
				{
					// Might have to convert channel name from Jis to ShiftJis, etc...
					if (!bConvertString(TRUE, m_byteCharSet, szSender, 0, &szSender, NULL, &bFreeConverted))
					{
						m_hrLastError = E_OUTOFMEMORY;
						return FALSE;
					}
			...
			{
				bRet = bFireTextMessage(NULL, szSender, NULL, pPrIrcMsg->szParams[1], 0L, (cmdidNotice == nCmd) ? pmtNotice : pmtNormal);
				if (bFreeConverted && szSender)
					delete [] szSender;
				return bRet;
			}
		}
	...
	if (pChannel)
	{
		...
		// if sender is the channel, we might have some conversion to make
		if (!lstrcmpi(szSender, pChannel->m_prChannel.szName))
		{
			if (bIsIrcXSocket())
			{
				if (g_chExtChnPfx == pChannel->m_prChannel.szName[0])
				{
					// Need to convert UTF8 extended channel name to Unicode
					if (!bConvertUTF8StringToWide(pChannel->m_prChannel.szName, 0 /*cchIn*/, &wszChannelName, NULL /*pcchOut*/, FALSE, TRUE))
					{
						m_hrLastError = E_OUTOFMEMORY;
						return FALSE;
					}
				...
				{
					// Might have to convert channel name from Jis to ShiftJis, etc...
					if (!bConvertString(TRUE, m_byteCharSet, pChannel->m_prChannel.szName, 0, &szSender, NULL, &bFreeConverted))
					{
						m_hrLastError = E_OUTOFMEMORY;
						return FALSE;
					}
		...
		bRet = bFireTextMessage(pChannel, szSender, NULL, pPrIrcMsg->szParams[1], (cmdidNotice == nCmd) ? msgtNotice : msgtNormal);

		if (szSender && bFreeConverted)
			delete [] szSender;
	...
	if (pChannel)
	{
		...
		switch (cpnProp)
		{
			...
			default:
				...
				if (*szPropNewValue && (cpnProp == cpnHostKey || cpnProp == cpnOwnerKey || cpnProp == cpnSubject || cpnProp == cpnOnJoin || cpnProp == cpnOnPart))
				{
					if (*szChannel == g_chExtChnPfx && cpnProp != cpnOnPart)
					{
						ASSERT(!pChannel->bMicOnly(), "pChannel->bMicOnly() is TRUE in CChatSocket::bHandlePropChanged");
						// Need to convert UTF8 extended channel host|ownerkey|subject to Unicode
						if (!bConvertUTF8StringToWide(szPropNewValue, 0 /*cchIn*/, &wszChannelProp, NULL /*pcchOut*/, FALSE /*bNickname*/, FALSE /*bChannelName*/, FALSE /*bPostProcess*/))
						{
							m_hrLastError = E_OUTOFMEMORY;
							return FALSE;
						}

						ASSERT(wszChannelProp, "wszChannelProp in NULL in CChatSocket::bHandlePropChanged");

						vNewChannelProperty.bstrVal = SysAllocString(wszChannelProp);
						delete [] wszChannelProp;
					}
					else
					{
						// Might have to convert property from Jis to ShiftJis, etc...
						if (!bConvertString(TRUE, GetCharSet(), szPropNewValue, 0, &szTmp, NULL, &bFreeConverted))
						{
							m_hrLastError = E_OUTOFMEMORY;
							return FALSE;
						}
		...
		if (*pszPropOldValue)
		{
			if (**pszPropOldValue && (cpnProp == cpnHostKey || cpnProp == cpnOwnerKey || cpnProp == cpnSubject || cpnProp == cpnOnJoin || cpnProp == cpnOnPart))
			{
				if (*szChannel == g_chExtChnPfx && cpnProp != cpnOnPart)
				{
					ASSERT(!pChannel->bMicOnly(), "pChannel->bMicOnly() is TRUE in CChatSocket::bHandlePropChanged");
					// Need to convert UTF8 extended channel host|ownerkey|subject to Unicode
					if (!bConvertUTF8StringToWide(*pszPropOldValue, 0 /*cchIn*/, &wszChannelProp, NULL /*pcchOut*/, FALSE /*bNickname*/, FALSE /*bChannelName*/, FALSE /*bPostProcess*/))
					{
						m_hrLastError = E_OUTOFMEMORY;
						return FALSE;
					}
					...
					vOldChannelProperty.bstrVal = SysAllocString(wszChannelProp);
					delete [] wszChannelProp;
				}
				else
				{
					// Might have to convert property from Jis to ShiftJis, etc...
					if (!bConvertString(TRUE, GetCharSet(), *pszPropOldValue, 0, &szTmp, NULL, &bFreeConverted))
					{
						m_hrLastError = E_OUTOFMEMORY;
						return FALSE;
					}
				...
				vOldChannelProperty.bstrVal = A2BSTR(*pszPropOldValue); 
		}
		else
...
BOOL CChatSocket::bHandleTopic(PPRIRCMSG pPrIrcMsg)
{
	...
	if (pChannel)
	{
		if (szNickname)	// TOPIC command case
		{
		...
		{
			// Reply 332 case - we found the channel and fire the OnChannelProperty event
			...
			// expose the information in the form of a OnChannelProperty event
			return bFireChannelProperty(pChannel, &(pChannel->m_prChannel));
		}
	}
	else
	{
	...
	*sz = g_chEOS;
	...
	else
	{
		...
		if (bKeywordSet)
		{
			...
			{
				if (g_chExtChnPfx == pChannel->m_prChannel.szName[0] && !pChannel->bMicOnly())
				{
					// Need to convert UTF8 extended channel keyword to Unicode
					if (!bConvertUTF8StringToWide(szModifiedNickname, 0 /*cchIn*/, &wszChannelKeyword, NULL /*pcchOut*/, FALSE /*bNickname*/, FALSE /*bChannelName*/, FALSE /*bPostProcess*/))
					{
						m_hrLastError = E_OUTOFMEMORY;
						return FALSE;
					}

					ASSERT(wszChannelKeyword, "wszChannelKeyword in NULL in CChatSocket::bHandleModeCommand");

					vNewProperty.bstrVal = SysAllocString(wszChannelKeyword);
					delete [] wszChannelKeyword;

					if (pChannel->m_prChannel.szKeyword && g_chEOS != pChannel->m_prChannel.szKeyword[0])
					{
						if (!bConvertUTF8StringToWide(pChannel->m_prChannel.szKeyword, 0 /*cchIn*/, &wszChannelKeyword, NULL /*pcchOut*/, FALSE /*bNickname*/, FALSE /*bChannelName*/, FALSE /*bPostProcess*/))
						{
							m_hrLastError = E_OUTOFMEMORY;
							return FALSE;
						}
						...
						vOldProperty.bstrVal = SysAllocString(wszChannelKeyword);
						delete [] wszChannelKeyword;
					}
					else
						vOldProperty.bstrVal = SysAllocString(L"");
				}
				else
				{
					// Might have to convert keyword from Jis to ShiftJis, etc...
					if (!bConvertString(TRUE, GetCharSet(), szModifiedNickname, 0, &szTmp, NULL, &bFreeConverted))
					{
						m_hrLastError = E_OUTOFMEMORY;
						return FALSE;
					}

					vNewProperty.bstrVal = A2BSTR(szTmp);

					if (bFreeConverted)
						delete [] szTmp;

					if (pChannel->m_prChannel.szKeyword)
					{
						if (!bConvertString(TRUE, GetCharSet(), pChannel->m_prChannel.szKeyword, 0, &szTmp, NULL, &bFreeConverted))
						{
							m_hrLastError = E_OUTOFMEMORY;
							return FALSE;
						}

						vOldProperty.bstrVal = A2BSTR(szTmp);
...
BOOL CChatSocket::bHandleListMsg(PPRQUERY pPrQry, PPRIRCMSG pPrIrcMsg, UINT uCode)
{
	...
	ASSERT(m_listListType.DwGetCellCount() == (DWORD) (m_lListInProgress + m_lChannelListingInProgress), "m_listListType.DwGetCellCount() == m_lListInProgress + m_lChannelListingInProgress in CChatSocket::bHandleListMsg");

	if (RPL_LISTSTART == uCode || RPL_LISTXSTART == uCode)
	{
		// Channel listing situation implies:
		//	- pPrQuery == NULL
		//	- m_lChannelListingInProgress > 0L

		// Channel property access situation implies:
		//	- RPL_LISTSTART == uCode
		//	- m_listQuery.DwGetCellCount() > 0L
		//	- m_lListInProgress > 0L

		#ifdef DEBUG
			BOOL	bChannelListingCase = (pPrQuery == NULL && m_lChannelListingInProgress > 0L);
			BOOL	bChannelPropAccessCase = (RPL_LISTSTART == uCode && m_listQuery.DwGetCellCount() > 0L && m_lListInProgress > 0L);

			sprintf(g_szDebugStr, "CChatSocket::bHandleListMsg m_listQuery.DwGetCellCount()=%d, m_lListInProgress=%d, m_lChannelListingInProgress=%d, m_listListType.DwGetCellCount()=%d, pPrQuery=%d\n", m_listQuery.DwGetCellCount(), m_lListInProgress, m_lChannelListingInProgress, m_listListType.DwGetCellCount(), pPrQuery);
			OutputDebugThreadIdString(g_szDebugStr);

			sprintf(g_szDebugStr, "CChatSocket::bHandleListMsg bChannelListingCase=%d, bChannelPropAccessCase=%d\n", bChannelListingCase, bChannelPropAccessCase);
			OutputDebugThreadIdString(g_szDebugStr);

			ASSERT(bChannelListingCase || bChannelPropAccessCase, "!bChannelListingCase && !bChannelPropAccessCase in CChatSocket::bHandleListMsg");
		#endif // DEBUG

		DWORD	dwListType;
		if (!m_listListType.bGetCell(1L, (PVOID*) &dwListType))
		{
			m_hrLastError = E_FAIL;
			return FALSE;
		}

		ASSERT(dwListType == (DWORD) g_byteChannelPropAccessType || dwListType == (DWORD) g_byteChannelListingType, "dwListType != g_byteChannelPropAccessType && dwListType != (DWORD) g_byteChannelListingType in CChatSocket::bHandleListMsg");

		if (dwListType == (DWORD) g_byteChannelPropAccessType)
		{
			// Channel property access situation
			OutputDebugThreadIdString("CChatSocket::bHandleListMsg - Channel property access situation\n");
			ASSERT(RPL_LISTSTART == uCode, "RPL_LISTSTART != uCode in CChatSocket::bHandleListMsg");
			bExpectingOneChannel = TRUE;
		}
		else
		{
			// Channel listing situation - Fire the OnBeginEnumeration event
			OutputDebugThreadIdString("CChatSocket::bHandleListMsg - Firing OnBeginEnumeration(etChannels)\n");
			ASSERT(!bFireEndEnum, "bFireEndEnum is TRUE in CChatSocket::bHandleListMsg");
			bFireEndEnum = TRUE;
	...
	if (RPL_LISTEND == uCode || RPL_LISTXEND == uCode || RPL_LISTXTRUNC == uCode)
	{
		if (bFireChannelProp)
		{
			...
			if (RPL_LISTEND != uCode)
			{
				ASSERT(m_prChannel.szName, "m_prChannel.szName is NULL in CChatSocket::bHandleListMsg");
				// This is a Channels Listing situation where we need to expose the last filled up channel
				if (m_prChannel.szName)	// REGISB 01/05/98 shouldn't be there
		...
		if (bFireEndEnum)
		{
				...
				ASSERT(m_listListType.DwGetCellCount() == (DWORD) (m_lListInProgress + m_lChannelListingInProgress), "m_listListType.DwGetCellCount() == m_lListInProgress + m_lChannelListingInProgress in CChatSocket::bHandleListMsg - RPL_LIST[X][TRUNC]END");
				DWORD	dwListType;
				m_listListType.bGetCell(1L, (PVOID*) &dwListType);
				ASSERT(dwListType == (DWORD) g_byteChannelListingType, "dwListType != g_byteChannelListingType in CChatSocket::bHandleListMsg");
			#endif // DEBUG

			m_lChannelListingInProgress--;
			m_listListType.bRemoveCellAt(1L);
		...
		{
			ASSERT(RPL_LISTEND == uCode, "RPL_LISTEND != uCode in CChatSocket::bHandleListMsg");

			#ifdef DEBUG
				ASSERT(m_lListInProgress > 0L, "m_lListInProgress <= 0L in CChatSocket::bHandleListMsg - RPL_LISTEND");
				ASSERT(m_listListType.DwGetCellCount() == (DWORD) (m_lListInProgress + m_lChannelListingInProgress), "m_listListType.DwGetCellCount() == m_lListInProgress + m_lChannelListingInProgress in CChatSocket::bHandleListMsg - RPL_LISTEND");
				DWORD	dwListType;
				m_listListType.bGetCell(1L, (PVOID*) &dwListType);
				ASSERT(dwListType == (DWORD) g_byteChannelPropAccessType, "dwListType != g_byteChannelPropAccessType in CChatSocket::bHandleListMsg");
			#endif // DEBUG
...
BOOL CChatSocket::bHandleClone(PPRIRCMSG pPrIrcMsg)
{
	...
	if (pChannel->m_prChannel.lCloneListCount > 0L)
	{
		...
		// We need to add the clone to the CloneList property
			...
			LONG lUBound;
			// Get the current vector size
			HRESULT hr = SafeArrayGetUBound(pChannel->m_prChannel.psaCloneList, 1, &lUBound);
