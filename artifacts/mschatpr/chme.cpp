...
{
	BOOL		bRet, bFreeConvertedCK = FALSE;
	...
	if (VT_BSTR == pvCK->vt && pvCK->bstrVal && *pvCK->bstrVal)
	{
		...
		{
			if (!bConvertWideStringToUTF8(pvCK->bstrVal, 0, &szChannelKey, NULL, FALSE /*bNickname*/, FALSE /*bChannelName*/, FALSE /*bPostProcess*/))
				return HrThrowError(CTL_E_OUTOFMEMORY);
			bFreeConvertedCK = TRUE;
		}
		else
		{
			if (!(szTmp = W2T(pvCK->bstrVal)) ||
				!bConvertString(FALSE /*bIncomingText*/, m_pccsock->GetCharSet(), szTmp, 0, &szChannelKey, NULL, &bFreeConvertedCK))
				return HrThrowError(CTL_E_OUTOFMEMORY);
		}
	}
	...
	bRet = m_pccsock->bCreateJoinChannel((CChatChannel*) this, szChannelKey, TRUE /*bCreate*/);

	if (bFreeConvertedCK && szChannelKey)
		delete [] szChannelKey;

	return bRet ? NOERROR : HrThrowError(GetScodeFromHResult(m_pccsock->HrGetLastError()), NULL, m_pccsock->SzGetNickname());
}
...
STDMETHODIMP CChannelObject::JoinChannel(VARIANT ChannelName, VARIANT ChannelKeyword)
{
	...
	if (VT_BSTR == pvCK->vt && pvCK->bstrVal && *pvCK->bstrVal)
	{
		if (g_chExtChnPfx == m_prChannel.szName[0])
		{
			// REGISB: the keyword should not be converted if user tries to join a MIC-only channel
			// ==> OCX user won't be able to join a MIC-only channel if its keyword is extended
			if (!bConvertWideStringToUTF8(pvCK->bstrVal, 0, &szChannelKey, NULL, FALSE /*bNickname*/, FALSE /*bChannelName*/, FALSE /*bPostProcess*/))
				return HrThrowError(CTL_E_OUTOFMEMORY);
			bFreeConvertedCK = TRUE;
		}
	...
	bRet = m_pccsock->bCreateJoinChannel((CChatChannel*) this, szChannelKey, FALSE /*bCreate*/);
