...
public:
	...
	CWndMsgLList*	GetPRedirList(void)
										{ return &m_listRedir; }

	CLList*			GetPListTypeList(void)
										{ return &m_listListType; }
...
private:
	...
	HRESULT			HrIterateLogin(PPRSECURITY pPrSecurity, HRESULT hrLatest, BOOL bForceNextPackage);
	...
	virtual BOOL	bFireChannelProperty(CChatChannel* pChannel, PPRCHANNEL pPrChannel) = 0;
...
protected:
	...
	CWndMsgLList			m_listRedir;		// Buffered messages for m_hwndMess
	CMaskLList				m_listMask;			// UserMatch Masks
	CQueryLList				m_listQuery;
	CChanLList				m_listChannel;		// channels list
	CLList					m_listListType;		// LIST(X) command type: channel listing or channel property access
