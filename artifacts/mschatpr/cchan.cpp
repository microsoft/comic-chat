		...
		switch (cpnProp)
		{
			...
			case cpnMemberCount:																  // We don't want to do a LISTX here
				...
				if (SUCCEEDED(m_hrLastError))
				{
					...
					m_pccsock->GetPListTypeList()->bInsertCell((LONG) m_pccsock->GetPListTypeList()->DwGetCellCount()+1, (PVOID) g_byteChannelPropAccessType);
				}
				break;
...
error:
	...
	if (pPrQuery)
		...
		{
			if (pPrQuery->uHeadReplyCode == RPL_LISTSTART && plProgressCount && *plProgressCount != lOldProgressCount)
			{
				ASSERT(*plProgressCount == lOldProgressCount + 1, "*plProgressCount != lOldProgressCount + 1 in CChatChannel::bQueryChannelProperty");
				// Remove the last cell we just added
				m_pccsock->GetPListTypeList()->bRemoveCellAt(0L);
			}
			if (plProgressCount)
...
BOOL CChatChannel::bChangeChannelProperty(LPCWSTR wszChannelPropertyName, VARIANT *pvChannelProperty)
{
	...
	switch (pvChannelProperty->vt)
	{
		case VT_BSTR:
			if (pvChannelProperty->bstrVal)
			{
				...
				// This might be a topic, keyword, ownerkey, hostkey, subject, onjoin or onpart that needs to be converted
				switch (cpnProp)
				{
					case cpnTopic:
					case cpnKeyword:
					...
					case cpnOnPart:
					{
						if (g_chExtChnPfx == m_prChannel.szName[0] && !m_prChannel.bMicOnly && cpnProp != cpnOnPart)
						{
	...
	switch (cpnProp)
	{
		...
		case cpnKeyword:
		{
			if (szValue)
			{																													  // Set	// Reset
				if (*szValue || (m_prChannel.szKeyword && *m_prChannel.szKeyword))
