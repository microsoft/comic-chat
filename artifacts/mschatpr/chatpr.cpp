...
LRESULT CALLBACK MsgWndProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	if (uMsg >= WM_U_MIN && uMsg <= WM_U_MAX)
	{
   		...
   		SCODE		sc = S_OK;
		static BOOL	bRedirEmpty = TRUE;

		ASSERT(pChatCtrl, "pChatCtrl is NULL in MsgWndProc");

		if (!pChatCtrl)
			return 0L;	// To be on the safe side
	
		if (pChatCtrl->m_nFreezeEvents > 0)
		{
			// Redirect this message to the redirection linked list because container asked to freeze the events
			// ASSERT(uMsg != WM_U_UNFREEZE, "uMsg == WM_U_UNFREEZE");
			if (uMsg != WM_U_UNFREEZE)
			{
				PPRMSG	pPrMsg = new PRMSG;
				ASSERT(pPrMsg, "pPrMsg is NULL in MsgWndProc");
				if (pPrMsg)
				{
					pPrMsg->uMsg = uMsg;
					pPrMsg->wParam = wParam;
					pPrMsg->lParam = lParam;
					if (pChatCtrl->GetPRedirList()->bInsertCell((LONG) pChatCtrl->GetPRedirList()->DwGetCellCount()+1L, (PVOID) pPrMsg))
						bRedirEmpty = FALSE;
					else
						delete pPrMsg;
				}
		...
		else
		{
			...
			{
				bRedirEmpty = TRUE;
				while (!pChatCtrl->GetPRedirList()->bIsEmpty())
				{
					PPRMSG	pPrMsg;

					if (pChatCtrl->GetPRedirList()->bGetCell(1L, (PVOID*) &pPrMsg))
					{
						ASSERT(pPrMsg, "pPrMsg is NULL in MsgWndProc");

						PRMSG prMsg;
						prMsg.uMsg   = pPrMsg->uMsg;
						prMsg.wParam = pPrMsg->wParam;
						prMsg.lParam = pPrMsg->lParam;
						
						pChatCtrl->GetPRedirList()->bRemoveCellAt(1L);
						ASSERT(pChatCtrl->m_nFreezeEvents <= 0, "pChatCtrl->m_nFreezeEvents > 0");
						pChatCtrl->bDispatchMessage(prMsg.uMsg, prMsg.wParam, prMsg.lParam);
					}
					else
						pChatCtrl->GetPRedirList()->bRemoveCellAt(1L);
				}
			...
			if (pChatCtrl->bDispatchMessage(uMsg, wParam, lParam))
				return 0L;
		}
	}
...
BOOL CMsChatPr::bDispatchMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	switch (uMsg)
	{
		case WM_U_BEGINENUMERATION:
			...
			FireBeginEnumeration((CChatChannel*) wParam, (enumEnumType) lParam);
			break;
		...
		case WM_U_CONNECTIONSTATE:
			...
			FireConnectionState((enumConnectionState) wParam);
			break;
		...
		case WM_U_CHANNELSTATE:
			...
			FireChannelState((CChatChannel*) wParam, (enumChannelState) lParam);
			break;
		...
		case WM_U_CHANNEL_HRESULT:
			...
			FireChannelError((CChatChannel*) wParam, NULL, (HRESULT) lParam);
			break;
		...
		case WM_U_SOCKET_HRESULT:
			...
			FireConnectionError((LPCTSTR) wParam, NULL, (HRESULT) lParam);
			break;

		case WM_U_IRC_MSG:
			// wParam = pointer to PRIRCMSG
			// lParam = shall we delete the PRIRCMSG from the pool?
			TreatIrcMsg((PPRIRCMSG) wParam, (BOOL) lParam);
			break;
		...
		case WM_U_USERPROP:
			...
			ExposeUserItems((PPRQUERY) wParam, (PPRUSER) lParam);
			break;
		...
		case WM_U_MEMBERPROP:
			...
			ExposeMemberItems((PPRQUERY) wParam, (BOOL) lParam);
			break;
		...
		case WM_U_CHANNELPROP:
			...
			// lParam = TRUE if pPrQuery needs to be removed from query list
			//			FALSE if pPrQuery just needs to be freed
			ExposeChannelItems((PPRQUERY) wParam, (BOOL) lParam);
			break;

		case WM_U_CHANNELPROP2:
			// wParam = pointer to PRCHANNEL
			// lParam = 0L
			ExposeChannelItems(NULL /*pChannel*/, (PPRCHANNEL) wParam, TRUE /*bFreeContent*/);
			break;

		case WM_U_UNFREEZE:
			// just there to trigger the buffered messages in linked list
			OutputDebugThreadIdString("WM_U_UNFREEZE msg\n");
			break;

		default:
			ASSERT(FALSE, "Unexpected uMsg in CMsChatPr::bDispatchMessage");
			return FALSE;
	}
	return TRUE;
}
