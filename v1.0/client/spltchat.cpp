...
{
	m_nPctLeft =80; // initial left side size of 80%
	...
	if (bAccept)
	{
		...
		if (cySay < nPixelsSayMin)
		{
			...
			if (cySay == 0) 
			{
				...
				cyClient = cyClient - m_cyBorder - nPixelsSayMin;
			}
			else
			{
				// Take the difference from the client pane.
				cyClient = cyClient - (nPixelsSayMin - cySay);
			}
		}
...
void CSplitSay::OnSize( UINT nType, int cx, int cy )
{
	if (m_bViewsCreated)
	{
		if (!m_bInitialSizingDone)
		{
			...
			SetRowInfo( 0,rect.bottom-rect.top-nPixelsSayMin-m_cySplitter-m_cyBorder-m_cyBorder, 0);
		...
		else
		{
			...
			else if (iPixelsShrunk < 0)
			{
				...
				if ((nPixelsSayMin - cySay) >= (-iPixelsShrunk))
				{
				...
				else
				{
					...
					cyClient += ((-iPixelsShrunk) - (nPixelsSayMin - cySay));
