...

{
    ...
    if ( JIS_len == -1 ) {
        ...
        JIS_len = strlen ( (const char *) pJIS ) + 1;
    }
    i = 0;
    re = 0;
    if ( SJIS_len == 0 ) {
...
#ifdef DBCS_DIVIDE
        if ( pcontext->dStatus0.nCodeSet == CODE_JPN_JIS ) {
            UCHAR cJIS = pcontext->dStatus0.cSavedByte;
            if ( pcontext->dStatus0.fESC ){
                if ( cJIS ){
                    if ( cJIS == KANJI_IN_1ST_CHAR &&
                         ( *pJIS == KANJI_IN_2ND_CHAR1 ||
                           *pJIS == KANJI_IN_2ND_CHAR2 )){
                        pcontext->blkanji0 = TRUE;
                        pJIS++;
                        i++;
                    } else if ( cJIS == KANJI_OUT_1ST_CHAR &&
                                ( *pJIS == KANJI_OUT_2ND_CHAR1 ||
                                  *pJIS == KANJI_OUT_2ND_CHAR2 )){
                        pcontext->blkanji0 = FALSE;
                        pJIS++;
                        i++;
                    } else
                        re += 2;
            ...
            pcontext->dStatus0.fESC = FALSE;
        }
#endif
        while ( i < JIS_len ) {
            if ( *pJIS == SO ) {   // Kana  Mode In?
                pcontext->blkana = TRUE;
                pJIS++;
                i++;
                continue;
            }
            if ( *pJIS == SI ) {   // Kana Mode Out ?
                pcontext->blkana = FALSE;
                pJIS++;
                i++;
                continue;
            }
            if ( pcontext->blkana == TRUE ) {
                pJIS++;
                i++;
                re++;
                continue;
            ...
            if ( *pJIS == ESC ) {
                ...
                if ( i == JIS_len - 1 || i == JIS_len - 2 ){
                    pcontext->dStatus0.nCodeSet = CODE_JPN_JIS;
                    pcontext->dStatus0.fESC = TRUE;
                    if( i == JIS_len - 2 )
                        pcontext->dStatus0.cSavedByte = *(pJIS+1);
                    break;
                }
#endif
                ...
                if ( blkanji == FALSE ) {
                    ...
                    if ( i == JIS_len - 1 ){
                        pcontext->dStatus0.nCodeSet = CODE_JPN_JIS;
                        pcontext->dStatus0.cSavedByte = *pJIS;
                        break;
                    }
#endif
                    if ( *pJIS == '*' ) {
...
#ifdef DBCS_DIVIDE
    if ( pcontext->dStatus.nCodeSet == CODE_JPN_JIS ) {
        UCHAR cJIS = pcontext->dStatus.cSavedByte;
        if ( pcontext->dStatus.fESC ){
            if ( cJIS){
                if ( cJIS == KANJI_IN_1ST_CHAR &&
                     ( *pJIS == KANJI_IN_2ND_CHAR1 ||
                       *pJIS == KANJI_IN_2ND_CHAR2 )){
                    pcontext->blkanji = TRUE;
                    pJIS++;
                    i++;
                } else if ( cJIS == KANJI_OUT_1ST_CHAR &&
                            ( *pJIS == KANJI_OUT_2ND_CHAR1 ||
                              *pJIS == KANJI_OUT_2ND_CHAR2 )){
                    pcontext->blkanji = FALSE;
                    pJIS++;
                    i++;
                } else {
                    *pSJIS = ESC;
...
#ifdef DBCS_DIVIDE
            if ( i == JIS_len - 1 || i == JIS_len - 2 ){
                pcontext->dStatus.nCodeSet = CODE_JPN_JIS;
                pcontext->dStatus.fESC = TRUE;
                if( i == JIS_len - 2 )
                    pcontext->dStatus.cSavedByte = *(pJIS+1);
                break;
            }
#endif
            if ( *(pJIS+1) == KANJI_IN_1ST_CHAR    && 
            ...
            *pSJIS = *pJIS;
            ...
            if ( pcontext->blkanji == FALSE ) {
                ...
                *pSJIS = *pJIS;
                ...
                if ( i == JIS_len - 1 ){
                    pcontext->dStatus.nCodeSet = CODE_JPN_JIS;
                    pcontext->dStatus.cSavedByte = *pJIS;
                    break;
                }
#endif
                if ( *pJIS == '*' ) {
...
// we are doing one short cut, assuming SJIS is never more than JIS
// we don't find the length required before making the call.
int OurJIS_to_ShiftJIS (UCHAR *pJIS, int JIS_len, UCHAR **ppSJIS, int ShiftJIS_len)
{
	// need to convert to sjis
	INT	nSJisLen;
	INT nJisLen;

	if ((nJisLen = JIS_len) == -1)
		nJisLen= _tcslen((TCHAR*)pJIS)+1;	// should include null termination in length

	CONV_CONTEXT ccontext;
	ZeroMemory(&ccontext,sizeof(CONV_CONTEXT));

	// SJIS string can atmost be equal to JIS string- RamuM
	if (ShiftJIS_len <= 0)
	{
		nSJisLen = nJisLen;
		*ppSJIS = (UCHAR*) new TCHAR[nSJisLen];
		if (!*ppSJIS)
			return(-1);
		ZeroMemory(*ppSJIS, nSJisLen);
	}
	else
	{
		nSJisLen = JIS_to_ShiftJIS(&ccontext, pJIS, nJisLen, NULL, 0);

		if (ShiftJIS_len <= nSJisLen)
			return(-1);
		nSJisLen = ShiftJIS_len;
	}

	if (!*ppSJIS)	// return actual length of string required
		return(JIS_to_ShiftJIS(&ccontext, pJIS, nJisLen, NULL, 0));

	int i = JIS_to_ShiftJIS(&ccontext, pJIS, nJisLen, (UCHAR*) *ppSJIS, nSJisLen);
	ASSERT(i > 0, "JIS_to_ShiftJIS failed in OurJIS_to_ShiftJIS");

	// we allocated a bufffer and an error occured during conversion
	if ((ShiftJIS_len <= 0) && i < 0)
	{
		delete [] *ppSJIS;
		*ppSJIS = NULL;
	}
	...
	{
		TCHAR *szTemp;
		szTemp = new TCHAR[nJisLen*2];

		if (!szTemp)
			return (-1);

		int j = ShiftJIS_to_JIS((UCHAR*) *ppSJIS, i, (UCHAR*) szTemp, nJisLen*2);
//		ASSERT(j > 0, "Debug: ShiftJIS_to_JIS failed in OurJIS_to_ShiftJIS");
		OutputDebugString("!! ShiftJIS_to_JIS failed in OurJIS_to_ShiftJIS !!\n");
