...
CImpIOleDocumentSite::CImpIOleDocumentSite(PCSite pSite, LPUNKNOWN pUnkOuter)
    {
	ATLTRACE( "CImpIOleDocumentSite::CImpIOleDocumentSite\n" );
...
STDMETHODIMP CImpIOleDocumentSite::QueryInterface(REFIID riid, void **ppv)
    {
	ATLTRACE( "CImpIOleDocumentSite::QueryInterface\n" );
...
STDMETHODIMP_(ULONG) CImpIOleDocumentSite::AddRef(void)
    {
    ++m_cRef;
	ATLTRACE( "CImpIOleDocumentSite::AddRef(%d)\n", m_cRef );
...
STDMETHODIMP_(ULONG) CImpIOleDocumentSite::Release(void)
    {
    --m_cRef;
	ATLTRACE( "CImpIOleDocumentSite::Release(%d)\n", m_cRef );
...
STDMETHODIMP CImpIOleDocumentSite::ActivateMe(IOleDocumentView *pView)
    {
	ATLTRACE( "CImpIOleDocumentSite::ActivateMe\n" );
    ...
    if (NULL==pView)
        ...
        if (FAILED(m_pSite->m_pObj->QueryInterface(IID_IOleDocument, (void **)&pDoc)))
            return E_FAIL;

        if (FAILED(pDoc->CreateView(m_pSite->m_pImpIOleIPSite, m_pSite->m_pIStream, 0, &pView)))            
