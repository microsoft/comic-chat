...
BOOL CInPlaceFrame::OnCreateControlBars(CFrameWnd* pWndFrame, CFrameWnd* pWndDoc)
{
	...
	if (!m_wndToolBar.Create(pWndFrame) || !m_wndToolBar.LoadToolBar(/*IDR_SRVR_INPLACE*/theApp.m_bDoCB32 ? IDR_NM_MAIN : IDR_MAINFRAME))
	{
	...
	m_wndTabBar.SetOwner(this);

	if (!m_wndTabBar.Create(NULL, "TabBar", WS_CHILD | WS_VISIBLE, CRect(0, 0, 100, 100), pWndFrame, 81)) {
	...
	pWndFrame->DockControlBar(&m_wndTabBar, AFX_IDW_DOCKBAR_TOP);
	cui.m_pvTabBar = &m_wndTabBar;
	if (theApp.m_bDoCB32) m_wndTabBar.ShowWindow(SW_HIDE);
#endif 
	return TRUE;
}
