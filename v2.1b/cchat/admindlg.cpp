...
void CBanDlg::DoBan(BOOL bBan) 
{
	...
	if (!strBanPattern.IsEmpty()) {
		currentRoom->ChatBanUser(strBanPattern, bBan);
		if (bBan) m_ctlBans.AddString(strBanPattern);
		else {
			int iIndex = m_ctlBans.FindStringExact(-1, strBanPattern);
			if (iIndex != LB_ERR) m_ctlBans.DeleteString(iIndex);
		}
	}
//	EndDialog(IDOK);
}
...
BOOL CBanDlg::OnInitDialog() 
{
	...
	for (int i = 0; i <= upper; i++)
		m_ctlBans.AddString(m_banArray->GetAt(i));
