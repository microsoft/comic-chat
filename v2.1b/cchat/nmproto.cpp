...
void CNmProto::SendMessageText(char *szAnnotations, char *szMessage, const char *szAddressee, char *szNMText /*= NULL*/) {
	...
	uBuffer = strlen( szMessage ) + sizeof( TCHAR );
	...
	if( IS_ANNOTATED( szMessage ) )
		...
		// look for end of annotation so we don't send it to less evolved NM chats
		TCHAR *pstr = (TCHAR *)_mbsstr( (const unsigned char *)szMessage, 
		...
		if( pstr != NULL )
			...
			// blow off leading blank if its there
			if( !IsDBCSLeadByte( (BYTE)*pstr ) && (*pstr == _T(' ')) )
				pstr = CharNext( pstr );
			}

		hr = m_pCbCore->
			SendMessageData( toInter, 
							 lstrlen( pstr ) + sizeof( TCHAR ), 
								(byte *)pstr,									// Just text for NM
							 uBuffer, (byte *)szMessage );						// Annotation for CChat
		}
	else
		{
		// normal message, use normal buffer
...
#if 0
void CNmProto::ChatGetIdentity(CUserInfo *pui) {
