...
BOOL CRoomList::OnInitDialog() 
{
	...
	m_strUser = m_persist->m_strUserFilter;		  // ... must be done before DDX (called by OnInitDialog)
...
void CRoomList::OnGoto() 
{
	...
	CRoom *room = (CRoom *) m_persist->m_rooms[roomNum];
	EndDialog(0);
	bSwitchToRoom(room->m_name);   // XXX - should use encoded name instead!!!
}
...
void CRoomListCtrl::OnItemchanged(NMHDR* pNMHDR, LRESULT* pResult) 
{
	NM_LISTVIEW* pNMListView = (NM_LISTVIEW*) pNMHDR;
	ASSERT(GetParent());
	GetParent()->GetDlgItem(IDC_GOTO)->EnableWindow(GetSelectedCount() == 1);
