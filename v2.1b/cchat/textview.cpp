...
void CTextView::TextLine(CUserInfo *pui, const char *szLine, BYTE byteMode, BYTE bbCooked, 
						 CDWordArray *prgdwFormatting)
{
	if (byteMode == SM_SAY && !strcmp(szLine, "<Chr>")) 
	...
	if (byteMode == SM_SAY)
		mesgType = mtNormal;
	else if (byteMode == SM_THINK)
		mesgType = mtThought;
	else if (byteMode == SM_WHISPER)
	{
		mesgType = mtWhisper;
		void GetAddressees(CUserInfo *, const char *, CString &, BOOL);
	...
	if (mesgType != mtEndEnum)
	{
		...
		m_textCore.iDisplayMsgHeader(cbLen, szFrom, 0, strTo, 0, mesgType, membStatus);
	...
	else if (byteMode == SM_ACTION)
	{
	//	const char *szAction = _tcschr(line, ' ');
	//	if (!szAction) 
	//		szAction = "";
	//	while (my_isspace(*szAction)) 
	//		szAction++;
...
void CTextView::DisplayPart(const char *nick) {
	CUserInfo *pui = LookupPui(nick);
	MEMBER_STATUS membStatus = msParticipant;
	if (pui && pui->IsOperator()) membStatus = msHost;
	if (pui) nick = pui->GetQualifiedName();
	m_textCore.iDisplayMemberStatus(nick, 0, mtLeave, membStatus);
}


void CTextView::DisplayJoin(const char *szNick)
{
	...
	m_textCore.iDisplayMemberStatus(szNick, 0, mtJoin, membStatus);
}
...
void CTextView::DisplayNickChange(CUserInfo *pui, const char *szOldNick)
{
	...
	if (pui)
	{
		...
		m_textCore.iDisplayInfo(DecodeNick(szOldNick), 0, UnConst(pui->GetScreenName()), 0,
...
//void CTextView::SetFormat(CHARFORMAT *cf) {
//	m_textCore.bSetTextViewDefaultFormat(cf);
//}
...
void CTextView::HandleLink ( NMHDR * pNotifyStruct, LRESULT * result ) {
	m_textCore.bHandleLink((ENLINK *) pNotifyStruct);
	*result = 0;
}
...
BOOL CTextView::OnNotify(WPARAM wParam, LPARAM lParam, LRESULT* pResult) 
{
	...
	if((pNMHDR->code == EN_MSGFILTER) && (pMSGFILTER->msg == WM_RBUTTONDOWN))
	{
		...
		menu.GetSubMenu(0)->CheckMenuRadioItem(0, 1, GetDocument()->m_bComicView ? 0 : 1,
											   MF_BYPOSITION);
		ASSERT(GetDocument());
		UINT iEnable = (GetDocument()->GetConnectionStatus() == CX_INCHANNEL) ? MF_ENABLED : MF_GRAYED;
		menu.GetSubMenu(0)->EnableMenuItem(ID_CHANNELPROPS, iEnable | MF_BYCOMMAND);
		iEnable = (GetDocument()->m_proto->m_dwModes & CM_NOFORMAT) ? MF_GRAYED : MF_ENABLED;
		menu.GetSubMenu(0)->EnableMenuItem(ID_VIEW_COMICS, iEnable | MF_BYCOMMAND);
...
void InitializeTextCores(BOOL resetOld, BOOL resetSay)
{
	...
	while (pos) 
	{
		...
		if (!doc->m_bComicView) 
		{
			...
			if (tv) 
				InitializeTextCore(&tv->m_textCore, resetOld, resetSay);
		}
	}
}
