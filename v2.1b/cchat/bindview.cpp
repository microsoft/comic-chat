...
HRESULT CDocObjectServerDoc::RestoreMenu( void )
{
	USES_CONVERSION;
	ASSERT_VALID(this);
	ASSERT(IsDocObject());
	...
	if (!GetFileTypeString(strFileType))
		return E_FAIL;
	AfxFormatString2(strTitle, AFX_IDS_OBJ_TITLE_INPLACE, AfxGetAppName(), strFileType);


	// need to get frame & doc window interfaces as well as other info
	CDocObjectIPFrameWnd* pFrameWnd = (CDocObjectIPFrameWnd*)m_pInPlaceFrame;
	RECT rcPosRect, rcClipRect;
	if ((hr = m_pViewSite->GetWindowContext(
		&pFrameWnd->m_lpFrame, &pFrameWnd->m_lpDocFrame,
		&rcPosRect, &rcClipRect, &pFrameWnd->m_frameInfo)) != NOERROR)
		{
		return( hr );
	...
	// send activate notification
	if ((hr = 	m_pViewSite->OnUIActivate()) != NOERROR)
		{
		return( hr );
		}


	// get rid of old shared menu
	if( pFrameWnd->m_hSharedMenu != NULL )
		{
		::DestroyMenu(pFrameWnd->m_hSharedMenu);
		pFrameWnd->m_hSharedMenu = NULL;
		}

	if( pFrameWnd->m_hOleMenu != NULL )
		{
		::OleDestroyMenuDescriptor(pFrameWnd->m_hOleMenu);
		pFrameWnd->m_hOleMenu = NULL;
		}
	...
	// set the active object
	LPOLEINPLACEACTIVEOBJECT lpActiveObject;
	lpActiveObject = (LPOLEINPLACEACTIVEOBJECT)GetInterface(&IID_IOleInPlaceActiveObject);
	...
	// reset shared menu
	pFrameWnd->m_lpFrame->
		SetMenu(pFrameWnd->m_hSharedMenu, pFrameWnd->m_hOleMenu, pFrameWnd->m_hWnd);
...
void CDocObjectServerDoc::OnDeactivateUI(BOOL bUndoable)
	{
