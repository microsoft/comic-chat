...
CAvatarX *LoadAvatarInfo(const char *avName) {
	...
	magicNum = read16(fp);
	if (magicNum != AF_MAGICNUM) return NULL;
	avType = read16(fp);
	version = read16(fp);

	// need to store a duplicate in AVFileRecs.  Be careful on reclaiming!
	const char *filename = strdup(avName);
...
BOOL CAvatarX::LoadBasics(int key, FILE *fp, const char *path) {
	...
	case AK_NAME:
		int c, nRead;
		char buff[_MAX_FNAME], *bptr;
		bptr = buff;
		nRead = 0;
		while ((c = fgetc(fp)) != EOF) {
			if (!c) break;
			if (++nRead < _MAX_FNAME) *bptr++ = c;
		}
		*bptr = '\0';
		m_name = strdup(buff);
		return TRUE;
