...
BOOL CUnitPanelPage::SetFonts(LOGFONT &logFont, COLORREF crTextColor)
{
	...
	m_fontMini = new CFont;
	m_fonts.AddHead(m_fontMini);
	lf2 = logFont;
	lf2.lfHeight = nFontHeightMini;
	if (!m_fontMini->CreateFontIndirect(&lf2))
		return FALSE;

	double reduction = abs(logFont.lfHeight) / 180.0;
	int doVKern = (strcmp(szPhysFaceName, "Comic Sans MS") == 0) ? 1 : 0;
	m_fiWNormal = new CFontInfo(m_fontBalloon, crTextColor, (int)(-70 * reduction * doVKern), (int)(60 * reduction * doVKern));
	m_fontInfos.AddHead(m_fiWNormal);
	m_fiWWhisper = new CFontInfo(m_fontWhisper, crTextColor, (int)(-70 * reduction * doVKern), (int)(60 * reduction * doVKern));
	m_fontInfos.AddHead(m_fiWWhisper);
	m_fiMini = new CFontInfo(m_fontMini, crTextColor, (int)(-50 * reduction * doVKern), (int)(40 * reduction * doVKern));
	m_fontInfos.AddHead(m_fiMini);
...
void CUnitPanelPage::DestroyFonts() {
	m_fontTextBig = NULL;    // gratuitous now, but allows for multiple calls in the future
	m_fontBalloon = NULL;
	m_fontWhisper = NULL;
	m_fontMini = NULL;
	m_fontTitle = NULL;
