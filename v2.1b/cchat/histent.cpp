...
SayEntry::SayEntry(CUserInfo *pui, const char *szMesg, CDWordArray *prgdwFormatting)
{
	m_pui = pui;
	...
	m_udi.m_byteMode= pui->m_udi.m_byteMode;
...
void SayEntry::Execute(int)
{
	...
	{
		GetTextView()->TextLine(m_pui, m_mesg, m_udi.m_byteMode, m_udi.m_bbCooked, m_prgdwFormatting);
		return;
	}
	...
	GetView()->GetDocument()->ProcessLine(m_pui->GetAvatarID(), m_mesg, m_udi.m_byteMode, m_udi.m_bbCooked, m_prgdwFormatting);
}
...
void SayEntry::FormatOtherArgs(CString &str)
{
	str.Format("(G:%d %d %d E:%d %d %d R:%d M:%d", m_udi.m_chGest, m_udi.m_chGestE, m_udi.m_chGestI,
				m_udi.m_chExpr, m_udi.m_chExprE, m_udi.m_chExprI, m_udi.m_bbReq, m_udi.m_byteMode);
...
SayEntry::SayEntry(CString &str, CChatDoc *doc) 
{
...
void JoinEntry::Execute(int mode)
{
	...
	if (!m_bTherePrior && theApp.m_bShowArrivals)
		if (!GetChatDoc()->m_bComicView) 
			GetTextView()->DisplayJoin(m_name);

	if (mode == HM_LIVE && !theApp.m_bNoRefresh) {
		if (m_pui->IsSelf()) {
			extern const char *GetMyCharacter();

			GetChatDoc()->m_proto->ChatAnnounceNewAvatar(GetMyCharacter());
			if (theApp.m_bAway)
...
void JoinEntry::WriteSelf(CArchive &ar) {
	...
	if (m_fullName) s.Format("%s\t%s\t%s\r\n", op, m_name, m_fullName);
	else s.Format("%s\t%s\r\n", op, m_name);
	ar.WriteString(s);
}

JoinEntry::JoinEntry(CUserInfo *pui, BOOL bPrior) {
	CString attedNick;
	pui->GetAttedNick(attedNick, FALSE);
	m_name = strdup(attedNick);
...
void PartEntry::Execute(int mode) {
	...
	extern void DisplayPart(const char *);
	if (mode == HM_LIVE)
		CIUserPart(m_name);

	// display the darn thing
	if (theApp.m_bShowArrivals)
		if (!GetChatDoc()->m_bComicView) (GetTextView())->DisplayPart(m_name);
}
...
void ChangeAvatarEntry::Execute(int mode) {
	...
	if (!m_pui) m_pui = LookupPui(m_name);

	if (!GetChatDoc()->m_bComicView) return;  // This is a no-op unless in comics mode
...
void ChangeBackDropEntry::Execute(int) {
	...
	if (GetChatDoc()->m_bComicView) SetBackDrop(m_backName);
}
...
//		TRACE("strIn is %s.\n", strIn);
	...
	if (!puiSelf)  // in case file is bad, or old format
		AddAndExecute(new JoinEntry(new CUserInfo(GetMyNickName())), this);  // pretend to join to set pui & puiSelf
...
BOOL DoEasterEgg() {
	...
	BOOL bProcessAddChannel(const char *, CRoomInfo *), ArtDirsOK();
	...
	bProcessAddChannel(strChanName, NewDefaultProto(NULL));
...
//			AddAndExecute(histEnt, doc);
			delete histEnt;
			if (!puiSelf)  // in case file is bad, or old format
				AddAndExecute(new JoinEntry(new CUserInfo(GetMyNickName())), doc);  // pretend to join to set pui & puiSelf
		}
