...
CHARFORMAT gDefHighLightedFormats[g_nHighlightedFormats] = {		// 240 == 12 pt * 20 twips/pt
								{ sizeof(CHARFORMAT), 
								CFM_COLOR | CFM_BOLD | CFM_SIZE,
								CFE_BOLD,
								240,0,RGB(128,128,128) },
								{ sizeof(CHARFORMAT), 
								CFM_COLOR | CFM_BOLD, 
								CFE_BOLD,
								0,0,RGB(128,128,128) },
...
//			OutputDebugThreadIdString("TVWndProc - Thumb Track False\n");
    ...
    LRESULT lRet = CallWindowProc(lpfnWinProc, hwnd, uMsg, wParam, lParam);

	// Things to do *after* default processing.

	if (uMsg == WM_SIZE && pCtrl != NULL && !pCtrl->m_bInTextAdd)
	{
		BOOL bScroll;
		int nAutoScroll = pCtrl->nGetAutoScroll ();
		if (!IsWindowVisible(hwnd))
			bScroll = TRUE;	// to make things simplier in this rare case
		else if (TEXT_VIEW_AUTOSCROLL_ALWAYS == nAutoScroll)
			bScroll	= TRUE;
		else if (TEXT_VIEW_AUTOSCROLL_NOSELECT == nAutoScroll)
		{
			bScroll = pCtrl->bGetLastAutoScroll ();
		}
		else
			bScroll = FALSE;
...
DWORD TEXTVIEWCLASSNAME::dwClearTextViewBuffer(DWORD dwMinCut)
{
	...
	if (dwMinCut == 0L)		// Clear the entire History
	{
		if (m_dwBuffSize)
		{
...
BOOL TEXTVIEWCLASSNAME::bSetDefaultHighlightFormat(CHARFORMAT *pCharFormat, SHORT nIndex)
{
	ASSERT (nIndex >= 0 && nIndex < g_nHighlightedFormats, "nIndex out of range in CTextView::bSetDefaultHighlightFormat");

	m_rgcfDefHighlights[nIndex] = pCharFormat? *pCharFormat : gDefHighLightedFormats[nIndex];
...
BOOL TEXTVIEWCLASSNAME::bSetHighlightFormat(CHARFORMAT *pCharFormat, SHORT nIndex)
{
	ASSERT (nIndex >= 0 && nIndex < g_nHighlightedFormats, "nIndex out of range in CTextView::bSetHighlightFormat");

	DumpCharFormat(pCharFormat, "In bSetHighlightFormat:\n");
	...
	if (!pCharFormat)
	{
		if (m_rgpcfHighlights[nIndex])
		{
			delete m_rgpcfHighlights[nIndex];
			m_rgpcfHighlights[nIndex] = NULL;
		}
		return TRUE;
	}
	...
	if (!m_rgpcfHighlights[nIndex])
	{
		if (NULL == (m_rgpcfHighlights[nIndex] = new CHARFORMAT))
			return FALSE;
	}
	
	*m_rgpcfHighlights[nIndex] = *pCharFormat;
...
BOOL TEXTVIEWCLASSNAME::bGetDefaultHighlightFormat(CHARFORMAT **ppCharFormat, SHORT nIndex)
{
	ASSERT (nIndex >= 0 && nIndex < g_nHighlightedFormats, "nIndex out of range in CTextView::bGetDefaultHighlightFormat");

	if (!ppCharFormat)
		return FALSE;

	*ppCharFormat = &m_rgcfDefHighlights[nIndex];
...
BOOL TEXTVIEWCLASSNAME::bGetHighlightFormat(CHARFORMAT **ppCharFormat, SHORT nIndex)
{
	ASSERT (nIndex >= 0 && nIndex < g_nHighlightedFormats, "nIndex out of range in CTextView::bGetHighlightFormat");

	if (!ppCharFormat)
		return FALSE;

	return (*ppCharFormat = m_rgpcfHighlights[nIndex]) != NULL;
}
...

{
	...
	if (MsgType == mtWhisper || MsgType == mtExChan)
	{
	...
	if (mtWhisper == MsgType || mtExChan == MsgType)
	{
	...
	if (iHighlightIndex < 0 && NULL == pCharFormat)
	{
		if (m_pMsgTypePropHead[MembFrom][MsgType])
								...
								INT cFormats)
{
	...
	BOOL bPrevInTextAdd = m_bInTextAdd;
	m_bInTextAdd = TRUE;
...
//		::LockWindowUpdate(m_hwnd);
	...
	if (!bScroll)	// Disable AutoVScroll
	{
		...
		// bVisible = IsWindowVisible(m_hwnd); // Bug #2187
		bVisible = GetWindowLong(m_hwnd, GWL_STYLE) & WS_VISIBLE;
		bShowWnd = TRUE;
	...
	if (!bAppend)
	{
		if (m_dwBuffSize)	// add a line feed if the history is not empty
		{
			...
			{
				::SendMessage(m_hwnd, EM_REPLACESEL, 0L, (LPARAM) (LPCTSTR) g_szCRLF);
				m_dwBuffSize+=2;
				#ifdef DEBUG
					DWORD cbRecvLen = ::SendMessage(m_hwnd, WM_GETTEXTLENGTH, 0L, 0L);
					m_dwBuffSize = cbRecvLen;	// resync to RichEdit's actual length (modern RichEdit stores CRLF as one char)
					...
					{
						LPTSTR szTmp[1025];
						::SendMessage(m_hwnd, WM_GETTEXT, (WPARAM) 1024, (LPARAM) szTmp);
					}
				#endif // DEBUG
			}
			else
	...
	{
		ASSERT(iHighlightIndex < g_nHighlightedFormats, "iHighlightIndex >= g_nHighlightedFormats in CTextView::iDisplayMsgText");
		if (m_rgpcfHighlights[iHighlightIndex])
			pCharFormat = m_rgpcfHighlights[iHighlightIndex];
		else
			pCharFormat = &m_rgcfDefHighlights[iHighlightIndex];
	}
	else
		if (NULL == pCharFormat)
		{
							...
							INT cFormats)
{
	...
	if (!(szText = new TCHAR[dwTextLen+1]))
		goto exitOOM;
...
//		if ((si.nMin+ si.nTrackPos + (int)si.nPage) < si.nMax)
		if ( m_bThumb || ((si.nPos + (int)si.nPage) < si.nMax))
			bEndInView = FALSE;
	}
	
	m_bLastAutoScroll = bEndInView;
...
//		}
	...
	else if ((bRet) && ((si.nPos + (int)si.nPage) < si.nMax))
	{
		#if 0 // This code, especially the si.nMax >= SHRT_MAX case, looks quite suspect.
		...
		//}
		#endif
		// Use this instead - why not, it works.
		::SendMessage(m_hwnd, WM_VSCROLL, SB_BOTTOM, 0L);
	}
#ifdef DEBUG
	else
...
BOOL TEXTVIEWCLASSNAME::bSetDefaultMsgTypeProperties()
{
	...
	for (ms = 0; ms < msEndEnum; ms++)
	{
		...
		bSetMessageFormat(&CharFormatTemp, mtExChan, (MEMBER_STATUS) ms, TRUE);		// Header
	}
...
BOOL TEXTVIEWCLASSNAME::bSetDefaultHighlightFormats()
{
	for (short n = 0; n < g_nHighlightedFormats; n++)
	{
		m_rgpcfHighlights[n] = NULL;
		m_rgcfDefHighlights[n] = gDefHighLightedFormats[n];
	}
	return TRUE;
}
...
// To Delete or Reset specific highlighting formats
BOOL TEXTVIEWCLASSNAME::bReSetDefaultHighlightFormats(BOOL bReset)
{
	ClearDefaultHighlightFormats();

	if (bReset)
		return bSetDefaultHighlightFormats();
...
void TEXTVIEWCLASSNAME::ClearDefaultHighlightFormats()
{
